      ...
      while ( $line ne "" )
      {
        ...
        elsif ($line =~ /(DocPipe|this|DMLogger|DocuMatrix|Core)\.log\.(debug|info|warning|error|fatal)\s*\(\s*([0-9]+)\s*,\s*(true|false)\s*,\s*("[^"]*"\s*|[^\s]+\s*)(,|\))/i)
        {
          ...
          if ($davor !~ /\s*\/\/\s*$/)
          {
            ...
            $line .= $davor . "$package.log.$func( \"$component\", \"$vfile\", $line_number, \"$git_version\", $trace_level, $concat, $my_id$nach_string";
            if ($func eq "debug")
            {
              $line .= "$danach }\n";
              $danach = "";
        ...
        elsif ( $line =~ /(DocPipe|this|DMLogger|DocuMatrix|Core)\.t\s*\(\s*("[^"]+"|[^\s]+)\s*,\s*("[^"]*"|[^\s]+)\s*(,|\))/i )
        {
          ...
          $line = $davor . "$package.t( \"$component\", $caption, \"$message\" $nach_string";
          $rest = $danach;
        }
        $line =~ s/\x01/\\"/g;
        print FOUT $line;
